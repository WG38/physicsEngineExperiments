...
//g++ -c mainV02.cpp -I"C:\Users\wgare\Documents\CPP_Lib\SFML-2.5.1\include"
//g++ mainV02.o -o physC2 -L"C:\Users\wgare\Documents\CPP_Lib\SFML-2.5.1\lib" -lsfml-graphics -lsfml-window -lsfml-system
...
void Simulation(float stepSize,std::vector<Blob>& bArray,std::vector<Stick>& sArray,float gAcc,int radius,bool containerState) {
 int r = radius;
...
int main()
{
    ...
    while (window.isOpen()) {
     ...
     //simulate next step
     if (!timeFreeze) {
      Simulation(dt,blobVec,stickVec,g,r,isContainerON);
    ...
    //secondary key listeners 
     ...
     // switch the container on / off 
     if (isCClicked) {
       isContainerON = !isContainerON;
