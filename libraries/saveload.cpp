...
// vectors containing the blob and stick data 
...
std::vector<Blob>& blobs,std::vector<Stick>& sticks) {
 ...
 //+ masterBlobStr 
 std::string masterString = nBStr + nSStr + TFStr + MCStr + CStr + CFStr + masterBlobStr +  masterStickStr;
...
std::tuple<int,int,bool,bool,bool,float,std::vector<Blob>,std::vector<sf::CircleShape>,std::vector<Stick>> fileLoad() {
      ...
      if(savefile.is_open()) {
        ...
        bool timeFrozen = std::stoi(outStrVec[2].erase(outStrVec[2].length()-1,1));
        bool magneticsOn = std::stoi(outStrVec[3].erase(outStrVec[3].length()-1,1));
        bool containerOn = std::stoi(outStrVec[4].erase(outStrVec[4].length()-1,1));
        float newTime = std::stof(outStrVec[5].erase(outStrVec[5].length()-1,1));
        ...
        for (int i=0;i < nBlobs;i++) {
         ...
         Blob newBlob;
         std::string currStr = outStrVec[6+i];
        ...
        for (int i=0;i < nSticks;i++) {
          Stick newStick;
          std::string currStr = outStrVec[6+nBlobs+i];
          
          std::size_t latestSemiPos = currStr.find(';');
          newStick.blobIdents.resize(2);
          newStick.blobIdents[0] = std::stoi(currStr.substr(7,latestSemiPos-7));
